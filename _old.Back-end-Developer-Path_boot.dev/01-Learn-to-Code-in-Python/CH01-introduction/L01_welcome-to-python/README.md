# **Welcome to Learn Python**

![welcome-to-python](https://storage.googleapis.com/qvault-webapp-dynamic-assets/course_assets/UbgRs9O-800x200.png)

- **Let's Build a Game**
  In this course you'll build features for a text-based RPG called "Fantasy Quest".
- **Assignment**
  1. Press the gray `▶ Run` button to run the code I wrote for you
  2. Press the gold `▶ Submit` button to run *and submit* the code
     Press the `-->` right arrow at the top-right of the screen to go to the next lesson.
